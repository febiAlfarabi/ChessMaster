...
public final class UndoButton extends Button{
	...
	public UndoButton(Context context, AttributeSet attrs) {
		...
		if (isInEditMode()) {
			...
			setWidth(weight);
			setHeight(weight);
			this.setBackgroundResource(R.drawable.bg_undo_button);
...
//			this.setBackgroundResource(JDesign.getInstance().getMthemes().getThemes().get(curentTheme).getBgUndoButtonSelected());
			setWidth(weight+5);
			setHeight(weight+5);
...
//			this.setBackgroundResource(JDesign.getInstance().getMthemes().getThemes().get(curentTheme).getBgUndoButton());
			setWidth(weight);
			setHeight(weight);
